    ...
    <div class="panel" v-for="b in store.unlockedProcessingBuildings" :key="b.id">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:6px;flex-wrap:wrap;gap:4px;">
        <span style="font-size:0.78em;">
          ...
          <span v-if="b.inputAmount < b.effectiveInput" style="color:var(--red);">
            （还需 {{ store.fmt(b.effectiveInput - b.inputAmount) }}）
          </span>
          <span v-else-if="b.outputAmount >= b.outputCap" style="color:var(--red);">
            （产物已满）
          </span>
        </span>
        <div style="display:flex;gap:4px;">
          <button class="btn-sm btn-green" :disabled="!b.canProcess" @click="store.processBuilding(b.id)">
            加工
          </button>
          <button
            class="upgrade-btn"
